...
thumb_name = []
got_thumb = 3
while ((thumb_name == []) and (got_thumb != 0)):
    ser.write(bytes("scanning for USB \r",'UTF-8'))
    sleep(1)
    got_thumb = got_thumb-1
...
if got_thumb == 0:
    got_cfg = 0
    ser.write(bytes("using dfault cfg.txt\r",'UTF-8'))
    sleep(1)
else:
    got_cfg = 5
    buf10 = "False"
    while ((buf10 == 'False') and (got_cfg != 0)):
        print ("/media/pi/",thumb_name_pure,"/CWM/cfg.txt")
    ...
    if got_cfg == 0:
        ...
        ser.write(bytes("cfg.txt detected    \r",'UTF-8'))
...
if got_cfg != 0:    
    ser.write(bytes(" reading USB cfg.txt\r",'UTF-8'))
    sleep(1)
    ...
    #with open("/home/pi/CWM_DATA/cfg.txt", 'r') as reader:
        ...
        this_off_cycle = 1                             # for usb present situations set this_off_cycle to 1
    ...
    ser.write(bytes("reading def cfg.txt \r",'UTF-8'))
    sleep(1)
...
connected = 5
while ((connect() == False) and (connected != 0)):
    print('waiting to connect ',connected)
...
if connected != 0:
    ...
    ser.write(bytes("****Not Connected****\r",'UTF-8'))
...
#                survey_state = 2        # go straight to survey in progress  ( armed reserved for dashboard frc survey button )
        ...
        if (sequence!=0):
            ...
            if(survey_state == 3):  # if survey just completed then prepare and send the data 
                ...
                if ((send_email == "yes") or (1==1)):  # alway send email for now ( if possible )
                    msg = MIMEMultipart()
                    msg['From'] = fromaddr
                    msg['To'] = toaddr
                    ...
                    buf1 = chain_name 
                    now = datetime.datetime.now()
                    buf = (now.strftime("%Y-%m-%d_%H_%M_%S"))

                    filename = (buf1 + '_' + buf + '.csv')  
                    ...
                    filename = "cfg.txt" 
                    attachment = open("/home/pi/CWM_DATA/cfg.txt", "rb")      #destz
                    part = MIMEBase('application', 'octet-stream')
                    part.set_payload((attachment).read())
                    encoders.encode_base64(part)
                    part.add_header('Content-Disposition', "attachment; filename= %s" % filename)
                    ...
                    if ((got_thumb != 0) and (got_cfg != 0)):
                        ...
                        shutil.copyfile(zipfilename, '/media/pi/' + thumb_name_pure + '/survey_folder/' + buf1 + '_' + buf + '.zip')
                        shutil.copyfile(csvfilename, '/media/pi/' + thumb_name_pure + '/survey_folder/' + buf1 + '_' + buf + '.csv')
                        body = "Chain_Name: "
                        buf = chain_name
                        body = body + buf
                        ...
                        summary_file = open( '/media/pi/' + thumb_name_pure + '/survey_folder/' + buf1 + '_summary_' + buf + '.txt', "w")

                        n = summary_file.write(body)
                        summary_file.close() 
                        ser.write(bytes("** Survey Complete **\r",'UTF-8'))
                        ser.write(bytes("********************\r",'UTF-8')) 
                        this_off_cycle = 1000
            ...
            if(survey_state == 2):  # if survey just starting then do the houskeeping to initialize
                ...
                if ((got_thumb != 0) and( got_cfg != 0)):            #
                    ser.write(bytes("*Survey in Progress**\r",'UTF-8'))
    ...
    if(survey_state == 2):  # if survey in progress then process linkski
        ...
        if (new_frame==2):
            ...
            r.save("/home/pi/mu_code/pink.png")
            ...
            print(last_good_lft_scan,y_left,last_good_rt_scan,y_right)
            ...
            ser.write(bytes("xxx",'UTF-8'))
            ser.write(bytes(downstream,'UTF-8'))
            ser.write(bytes("\r",'UTF-8'))
