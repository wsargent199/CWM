...
while(True):
    ...
    #RPi_addr = "   % 1.3fin" % (millisec1 - millisec)
        ...
        if (sequence!=0):
            ...
            if(survey_state == 3):  # if survey just completed then prepare and send the data
                ...
                if send_email == "yes":
                    ...
                    buf   = "\rObserved chain length = %d links\r" %(last_w-1)
                    body  = body + buf
                    ...
                    filename = csvfilename     #"/home/pi/CWM_DATA/OP2_result_1.csv"
                    attachment = open(csvfilename, "rb")   #open("/home/pi/CWM_DATA/OP2_result_1.csv", "rb")
                    ...
                    filename = "/home/pi/mu_code/measureGS.py"  #"/home/pi/CWM_DATA/cfg.txt"
                    attachment = open("/home/pi/mu_code/measureGS.py", "rb")  #open("/home/pi/CWM_DATA/cfg.txt", "rb")
                    part = MIMEBase('application', 'octet-stream')
                    part.set_payload((attachment).read())
                    encoders.encode_base64(part)
                    part.add_header('Content-Disposition', "attachment; filename= %s" % filename)
    ...
    if(survey_state == 2):  # if survey in progress then process linkski
        ...
        raw_image_file_name = ("/home/pi/CWM_DATA_BAK/raw_%d.png" %(w))


        cv2.imwrite((raw_image_file_name),frame)

        millisec1 = int(round(time.time() * 1000))   # take time snapshot
        print ("just did frame grab",(millisec1 - millisec))
        ...
        if (new_frame==2):
            ...
            (thresh, blackAndWhiteImage) = cv2.threshold(gray, 250, 255, cv2.THRESH_BINARY)
            flipped = cv2.flip(blackAndWhiteImage,0)
            ...
            # /home/pi/CWM_HONDA/_home_pi_CWM_DATA_link_images
            # honda_FN = ("/home/pi/CWM_HONDA/_home_pi_CWM_DATA_link_images/boo2_%d.png"  %(w))
            #im = Image.open(honda_FN) # Can be many different formats.
